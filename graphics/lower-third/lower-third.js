...
async function updateDisplay() {
    ...
    if (team1.logo) {
        $(`.team1 > .teamName > .teamLogo`).get(0).style.setProperty("--teamLogo", `url(${team1.logo})`);
        $(`.team1 > .teamName > .teamLogo`).show();
    } else
        $(`.team1 > .teamName > .teamLogo`).hide();
    if (team2.logo) {
        $(`.team2 > .teamName > .teamLogo`).get(0).style.setProperty("--teamLogo", `url(${team2.logo})`);
        $(`.team2 > .teamName > .teamLogo`).show();
    } else
        $(`.team2 > .teamName > .teamLogo`).hide();


    ["Control", "Hybrid", "Assault", "Escort"].forEach((val) => { $(`.${val}`).removeClass(val) })
    for (let map = 0; map < 7; map++) {
        if (loadedMatch.maps[map]) {
            ...
            if (mapD.map != "None") {
                ...
                $(`#map${map + 1}display`).addClass(mapInfo.gamemode == "Upcoming" ? mapD.map : mapInfo.gamemode)
